...
class Downloader
{
    ...
    private const TRANSIENT_PREFIX = BIBCITE_PREFIX . "_Downloader_";
    ...
    public static function save_url_to_file(
        ...
        $last_downloaded_time_transient_name = 
            $transient_prefix . "last-downloaded-time";
        $last_downloaded_time_string = 
            get_transient($last_downloaded_time_transient_name);        
        Logger::instance()->info(
            "URL last downloaded: " . $last_downloaded_time_string
        );
        ...
        $new_etag = array_key_exists("etag", $headers) ? 
            $headers["etag"][0] : null;
    ...
    public static function uninstall() {
        ...
        foreach ($transients as $transient) {
            ...
            // Strip away the WordPress prefix in order to arrive at the 
            // transient key, then delete via the API.
            $key = str_replace( '_transient_timeout_', '', $transient );
            delete_transient($key);      
        }
        
        // But guess what? Sometimes transients are not in the DB, so we have 
        // to do this too:
        wp_cache_flush();
    }
}